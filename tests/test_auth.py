...
def test_mlflow_314_request_authorization_rules_cover_new_endpoints():
    ...
    label_schema_rules = {
        ...
        GetLabelSchemaByName: AuthorizationRule(
            "get", resource=RESOURCE_EXPERIMENTS,
            resource_name_parsers=experiment_id_parsers,
        ),
    ...
    # --- Path-based rules ---
    evaluate_invoke_rule = PATH_AUTHORIZATION_RULES[
        ("/ajax-api/3.0/mlflow/genai/evaluate/invoke", "POST")
    ]
    assert evaluate_invoke_rule == AuthorizationRule(
        "update", resource=RESOURCE_EXPERIMENTS,
        resource_name_parsers=experiment_id_parsers,
    )

    responses_compact_rule = PATH_AUTHORIZATION_RULES[
        ("/gateway/openai/v1/responses/compact", "POST")
    ]
    assert responses_compact_rule == AuthorizationRule(
        "create",
        resource=RESOURCE_GATEWAY_ENDPOINTS,
        subresource="use",
        resource_name_parsers=(RESOURCE_NAME_PARSER_GATEWAY_PROXY_ENDPOINT_NAME,),
